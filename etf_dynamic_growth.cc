...
class Growth {
 public:
  ...
  void Generate() {
    ...
    vector<vector<double>> interests(100);
    for (int past = 0; past < interests.size(); past++) {
      double ratio = log(1.005);
      double sum = 0.0;
      int range = 0;
      ...
      for (int index = 0; index < prices_.size(); index++) {
        ...
        sum += abs(position - old_position) * log(0.9985) / 2;
        // transaction += abs(position - old_position);
        result.push_back(sum);
      }
      interests[past] = result;
    }
    for (const auto& parameter :
      ...
      for (int index = 0; index < prices_.size(); index++) {
        ...
        sum += position * difference;
        int old_position = position;
        if (past == 0 || index - past < 0) {
          position = 1;
        } else {
          double max_interest = 0.0;
          int max_past = 0;
          for (int dynamic_past = 0; dynamic_past < interests.size();
               dynamic_past++) {
            if (dynamic_past != 0 && dynamic_past < 50) continue;
            double interest = interests[dynamic_past][index] -
                              interests[dynamic_past][index - past];
            if (interest * 1.001 > max_interest) {
              max_interest = interest;
              max_past = dynamic_past;
            }
          }
          if (max_past == 0) {
            position = 1;
            ...
            double old_price =
                (accumulated_prices_[index] -
                 accumulated_prices_[index - max_past]) / max_past;
            if (prices_[index] < old_price - ratio) {
              position = -1;
            } else if (prices_[index] > old_price + ratio) {
              position = 1;
